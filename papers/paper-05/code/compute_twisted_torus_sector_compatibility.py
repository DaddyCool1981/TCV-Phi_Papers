#!/usr/bin/env python3
"""Robustness checks for twisted-torus geometry across fermion sectors."""
...
from tcvphi.flavor_twisted_torus import (  # noqa: E402
    ...
    build_twisted_torus_hamiltonian,
    diagonalize,
...
def _load_best_twisted_candidate(path: Path) -> tuple[TwistedTorusParams, dict]:
    d = json.loads(path.read_text(encoding="utf-8"))
    best = d["best_candidates"][0]
    p = TwistedTorusParams(**best["params"])
    c = best["controls"]
    return p, c
...
def main() -> None:
    ...
    twisted_summary = out_data / "twisted_torus_flavour_toy_summary.json"
    if not twisted_summary.exists():
        raise FileNotFoundError("Run compute_twisted_torus_flavor_toy.py first.")

    p_best, c_best = _load_best_twisted_candidate(twisted_summary)

    # Fixed geometry around best candidate.
    h = build_twisted_torus_hamiltonian(p_best)
    vals, vecs = diagonalize(h)

    rng = np.random.default_rng(20260312)

    # Test A: leptons charged-sector robustness with same neutrino geometry.
    n_lepton = 3000
    lepton_angles = []
    lepton_ok = 0
    ...
    # Test B: quark-like sector from same geometry (small-mixing expectation).
    # Build two hierarchical sectors with tiny off-diagonal couplings.
    n_quark = 3000
    q12, q13, q23 = [], [], []
    ...
    summary = {
        "status": "twisted-torus sector compatibility check (exploratory)",
        "reference_candidate": {
            "params": p_best.__dict__,
            "controls": c_best,
        },
        "lepton_robustness": {
        ...
        "notes": [
            ...
            "It checks whether twisted-torus neutrino success is stable against charged-lepton perturbations.",
            "And whether a small-mixing quark-like toy can coexist on the same geometry.",
        ],
    }

    out_json = out_data / "twisted_torus_sector_compatibility_summary.json"
    out_json.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print("[INFO] Wrote:", out_json)

    # Figure 1: lepton robustness cloud.
    plt.figure(figsize=(6.2, 4.2))
    plt.scatter(l12, l13, s=10, alpha=0.25)
    plt.xlabel(r"$\theta_{12}$ (deg)")
    plt.ylabel(r"$\theta_{13}$ (deg)")
    plt.title("Twisted-torus lepton robustness cloud")
    plt.grid(ls=":", alpha=0.4)
    plt.tight_layout()
    f1 = out_figs / "twisted_torus_lepton_robustness_cloud.png"
    plt.savefig(f1, dpi=180, bbox_inches="tight")
    plt.close()
    print("[INFO] Wrote:", f1)

    # Figure 2: quark-like angle distributions.
    ...
    plt.ylabel("count")
    plt.title("Twisted-torus quark-like mixing toy")
    plt.grid(ls=":", alpha=0.4)
    plt.legend()
    plt.tight_layout()
    f2 = out_figs / "twisted_torus_quark_like_hist.png"
