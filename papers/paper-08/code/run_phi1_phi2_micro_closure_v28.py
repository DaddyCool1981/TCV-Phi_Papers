#!/usr/bin/env python3
"""v28: micro-closure scan with topology-linked potential softening + kinetic terms."""
...
def main() -> None:
    ...
    twist_grid = [0.0, 0.1, 0.2, 0.3]
    loop_grid = [0.0, 0.2, 0.4]
    psoft_grid = [0.0, 0.02, 0.05, 0.08, 0.12]

    records = []
    for i, eta in enumerate(eta_grid):
        for j, etw in enumerate(twist_grid):
            for kidx, eloop in enumerate(loop_grid):
                for lidx, ps in enumerate(psoft_grid):
                    tw_cfg = MicroClosureConfig(
                        ...
                        inertia_loop=float(eloop),
                        potential_twist=float(ps),
                    )
                    ...
                    tw = evaluate(tw_cfg, seed=20265000 + 2000 * i + 300 * j + 60 * kidx + lidx)
                    rg = evaluate(rg_cfg, seed=20266000 + 2000 * i + 300 * j + 60 * kidx + lidx)
    ...
    best_ratio = min(records, key=lambda r: r["m0sq_ratio_tw_over_ring"])

    out = {
        "best_v21_params": p,
        "grid": {
            "inertia_eta": eta_grid,
            "inertia_twist": twist_grid,
            "inertia_loop": loop_grid,
            "potential_twist": psoft_grid,
        },
        "n_records": len(records),
        "n_all_pass": len(pass_records),
        "best_ratio_record": best_ratio,
        "pass_records": pass_records[:30],
        "verdict": "micro_closure_supported_v28" if len(pass_records) > 0 else "micro_closure_still_open_v28",
        "records": records,
    }

    out_path = out_data / "phi1_phi2_micro_closure_v28_summary.json"
