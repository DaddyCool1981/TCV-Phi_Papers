#!/usr/bin/env python3
"""v27: micro-closure scan with loop/chirality-inspired kinetic term."""
...
def main() -> None:
    ...
    twist_grid = [0.0, 0.1, 0.2, 0.3, 0.4]
    loop_grid = [0.0, 0.1, 0.2, 0.4, 0.6, 0.8]
    ...
    for i, eta in enumerate(eta_grid):
        for j, etw in enumerate(twist_grid):
            for kidx, eloop in enumerate(loop_grid):
                ...
                tw = evaluate(tw_cfg, seed=20263000 + 500 * i + 50 * j + kidx)
                rg = evaluate(rg_cfg, seed=20264000 + 500 * i + 50 * j + kidx)
    ...
    out = {
        "best_v21_params": p,
        "grid": {"inertia_eta": eta_grid, "inertia_twist": twist_grid, "inertia_loop": loop_grid},
        "n_records": len(records),
        "n_all_pass": len(pass_records),
        "best_ratio_record": best_ratio,
        "pass_records": pass_records[:20],
        "verdict": "micro_closure_supported_v27" if len(pass_records) > 0 else "micro_closure_still_open_v27",
        "records": records,
    }

    out_path = out_data / "phi1_phi2_micro_closure_v27_summary.json"
